...
app.post("/admin/addConsti", async (req, res) => {
    try {
        ...
        var query3 = "INSERT INTO diversity VALUES (?, 0, 0, 0, 0, 0)";
...
app.post("/admin/getResultsOfYear", async (req, res) => {
    try {
        var query = "SELECT results.*, candidate.f_name, candidate.l_name, candidate.ruling, consti.consti_name, party.p_name " +
...
app.post("/admin/setResultsOfYear", async (req, res) => {
    try {
        ...
        var query2 = "CALL set_ruling(?)";
        var reply2 = await sqlQuery(query2, [req.body.latest_year]);
        res.status(200).json({ success: true });
    } catch (error) {
        console.log(error);
        res.status(401).json({ success: false });
